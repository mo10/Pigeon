...
#define USBD_PID_FS     0x2700
#define USBD_PRODUCT_STRING_FS     "Pigeon"
#define USBD_CONFIGURATION_STRING_FS     "Pigeon Cage"
#define USBD_INTERFACE_STRING_FS     "Pigeon Keyboard"
#define USBD_INTERFACE_STRING_FS2     "Pigeon LCD Panel"
...
__ALIGN_BEGIN uint8_t USBD_WINUSB_OSPropertyDesc[USB_LEN_OS_PROPERTY_DESC] __ALIGN_END =
{
  ...
  'D', 0,
  0, 0,
  // {CF040E5C-168B-48BA-BC33-665AC1CEB3B6}
...
uint8_t * USBD_FS_InterfaceStrDescriptor2(USBD_SpeedTypeDef speed, uint16_t *length);
...
, USBD_FS_InterfaceStrDescriptor2
#if (USBD_SUPPORT_WINUSB == 1) 
...
uint8_t * USBD_FS_InterfaceStrDescriptor2(USBD_SpeedTypeDef speed, uint16_t *length)
{
  if(speed == 0)
  {
    USBD_GetString((uint8_t *)USBD_INTERFACE_STRING_FS2, USBD_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)USBD_INTERFACE_STRING_FS2, USBD_StrDesc, length);
